...
from src.models.process_data import noise_removal,remove_borders
...
def test_remove_borders(raw_images_path):
    img = cv2.imread(raw_images_path[0]) 
    cut_image=remove_borders(raw_images_path[0])
    assert cut_image is not None
    assert (img.shape == cut_image.shape)
    assert (np.bitwise_xor(img,cut_image).any())
